...
public class Grid : MonoBehaviour
{
    ...
    void CreateGrid()
    {
        ...
        Vector3 bottomLeft = transform.position - Vector3.right * vGridWorldSize.x / 2 - Vector3.down * vGridWorldSize.y / 2;//Get the real world position of the bottom left of the grid.
        for (int x = 0; x < iGridSizeX; x++)//Loop through the array of nodes.
        {
            for (int y = 0; y < iGridSizeY; y++)//Loop through the array of nodes
            {
                Vector3 worldPoint = bottomLeft + Vector3.right * (x * fNodeDiameter + fNodeRadius) + Vector3.down * (y * fNodeDiameter + fNodeRadius);//Get the world co ordinates of the bottom left of the graph
    ...
    */
    private void OnDrawGizmos(){
        Gizmos.DrawWireCube(transform.position, new Vector3(vGridWorldSize.x, vGridWorldSize.y));
        if (NodeArray != null){
